...
RSpec.feature 'Candidate provides feeback throughout the application process' do
  ...
  def then_i_see_the_references_section_feedback_page
    expect(page).to have_current_path candidate_interface_application_feedback_path(path: candidate_interface_references_start_path)
  end
  ...
  def and_my_first_set_of_feedback_has_been_collected
    expect(@application.application_feedback.count).eq 1
    ...
    expect(@application.application_feedback.last.do_not_understand_section).to eq true
    ...
    expect(@application.application_feedback.last.other_feedback).to eq 'Me no undestand.'
    expect(@application.application_feedback.last.consent_to_contact).to eq true
  end
  ...
  def and_my_second_set_of_feedback_has_been_collected
    expect(@application.application_feedback.count).eq 2
    expect(@application.application_feedback.last.section).to eq 'application_references'
    expect(@application.application_feedback.last.path).to eq 'candidate_interface_references_start_path'
    expect(@application.application_feedback.last.page_title).to eq 'Choose a type of referee'
    expect(@application.application_feedback.last.issues).to eq false
    expect(@application.application_feedback.last.do_not_understand_section).to eq nil
    expect(@application.application_feedback.last.need_more_information).to eq nil
    expect(@application.application_feedback.last.answer_does_not_fit_format).to eq nil
    expect(@application.application_feedback.last.other_feedback).to eq nil
    expect(@application.application_feedback.last.consent_to_contact).to eq nil
  end
end
